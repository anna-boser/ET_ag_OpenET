...
filter(bardata, type == "ag_ET", cropnames != "Young Perennial") %>% 
  ...
  geom_linerange(data=filter(bardata, type == "ag_ET", cropnames != "Young Perennial"), aes(ymax = min, ymin = max), color = "#009966") + 
  ...
  scale_fill_manual(values=c(ag_ET="#009966", ET_pred="darkgoldenrod"), breaks=c("ag_ET","ET_pred"), labels = c("Agricultural ET", "Naturally-occurring ET")) + 
  ...
  geom_linerange(aes(ymax = min, ymin = max), size=1.1, color = "#009966") + 
  ylab("mm/year") + 
  theme_bw() + 
  labs(fill='') + 
  # ylim(c(-35, 165)) + 
...
ggplot(filter(bardata, type == "ag_ET"), aes(x = reorder(cropnames, ET))) +
  geom_linerange(data = filter(bardata, type == "ag_ET"), aes(ymax = min, ymin = max), color = "#009966") +
  geom_col(data = filter(bardata, type != "ET"), aes(y = ET, fill = type), alpha = .6) +
  scale_fill_manual(values=c(ag_ET="#009966", ET_pred="darkgoldenrod"), breaks=c("ag_ET","ET_pred"), labels = c("Agricultural ET", "Simulated natural ET")) +
...
ggplot(filter(bardata, type == "ag_ET"), aes(x = reorder(cropnames, ET))) + 
  geom_linerange(aes(ymax = min, ymin = max), color = "#009966") +
  geom_boxplot(data=data, aes(y=ag_ET), outlier.size = .5, outlier.alpha = .002) + 
  geom_col(aes(y = ET), fill = "#009966", alpha = 0.6) + 
  geom_linerange(aes(ymax = min, ymin = max), color = "#009966") +
  theme_bw() + 
  labs(fill='') + 
...
ggplot(filter(bardata, type == "ag_ET", subcropnames != "Young Perennial"), aes(x = reorder(subcropnames, ET))) + 
  geom_linerange(data = filter(bardata, type == "ag_ET", subcropnames != "Young Perennial"), aes(ymax = min, ymin = max), color = "#009966") + 
  ...
  geom_linerange(data = filter(bardata, type == "ag_ET", subcropnames != "Young Perennial"), aes(ymax = min, ymin = max), size = 1, color = "#009966") + 
  ylab("mm/year") + 
  theme_bw() + 
  labs(fill='') + 
  # ylim(c(-45, 175)) + 
