...
data$cluster <- mapply(assign_cluster, data$x, data$y, data$year, 75)
```
...
```{r}
ggplot(filter(bardata, type == "ag_ET"), aes(x = reorder(cropnames, ET))) + 
  geom_violin(data=data, aes(y=ag_ET))

# what about after controlling for PET/
```


# are there some people that consume way more than others, even contolling for crop type and climate? How about location (use county)?
```{r}
felm(ag_ET~cropnames | 0 | 0 | cluster+year, data) %>% summary()
felm(ag_ET~cropnames+PET | 0 | 0 | cluster+year, data) %>% summary()
felm(ag_ET~cropnames+ET_pred | 0 | 0 | cluster+year, data) %>% summary()
felm(ag_ET~cropnames+ET_pred+NAME | 0 | 0 | cluster+year, data) %>% summary()
```

# if all farmers that consume above average for their crop type, county, and ET_pred on any given year only consumed average, how much water would we save? 
```{r}
