...
fallow <- filter(fallow, year == 2019)
```

### A scatterplot with months and years averaged out
```{r}
if (months_ts){
...
ggplot(fallow_year) +
  geom_jitter(aes(x=ET_pred, y=ET, col=y), alpha=0.2, size =.1) + 
  theme_classic() + 
  geom_abline(intercept=0,slope=1, color="red") + 
  annotate("text", x=2*30, y=1.5, label= paste("R2:", round(r2, 3), "Bias:", round(bias, 3))) + 
...
## fallow bias plotted out in space
```{r}
if (months_ts){
  # plot out the time series
  fallow_ts <- fallow %>% group_by(x,y) %>% summarize(ET = mean(ET, na.rm=TRUE), 
                                                         ET_pred = mean(ET_pred, na.rm=TRUE))
  
  ggplot(fallow_ts) + 
    geom_point(aes(x=x, y=y, col=ET_pred-ET))
}
```
