...
data_natural <- fread(here("data", "3_for_counterfactual", "training_data", "fveg.csv"))

if (months_ts){
  # get a nice column of numeric months
  months <- select(data_natural, names(data)[grepl("month", names(data))])
  data_natural$month <- names(months)[max.col(months)] 
  data_natural$month <- str_extract(data$month, '(?<=_)\\d+') %>% as.numeric()
}
