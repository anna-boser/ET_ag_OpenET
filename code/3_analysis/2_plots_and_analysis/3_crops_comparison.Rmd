...
if (months_ts == TRUE){
  ...
  data <- fallow %>% group_by(x, y) %>% summarize(ET = mean(ET, na.rm=TRUE), 
...
ggplot(ag_ET_df, aes(x = reorder(cropnames, ET))) + 
  ...
  theme_classic() + 
  labs(fill='') + 
  geom_hline(yintercept = avg_irr, linetype = "dotted", color = "red", size = "0.5") + 
  annotate("text", x="Grain and hay crops", y=avg_irr + .25, label= "Average water diverted for irrigation") + 
