...
loc_df %>% filter(type %in% c("ET", "ET_pred")) %>%
  ...
  scale_color_distiller(name="ET (mm/year)", palette="RdBu") + 
...
loc_df %>% filter(type %in% c("Bias")) %>%
  ...
  scale_color_distiller(name="Bias (mm/year)", palette="RdBu", limits = c(-lim, lim)) + 
  theme_void()+ 
  xlim(c(minlong, maxlong)) + 
  ylim(c(minlat, maxlat)) + 
...
ggplot(ts, aes(x = month)) + 
  ...
  scale_color_viridis(discrete = TRUE) +  # Colorblind-friendly palette for lines
  scale_fill_viridis(discrete = TRUE) +   # Colorblind-friendly palette for ribbons
  ylab("ET (mm/year)") + 
  xlab("Month") + 
  labs(color=NULL, fill=NULL) + 
  theme_bw() + 
  theme(legend.position = "top", legend.direction = "horizontal") # move legend to the top to allow more space for the plot
