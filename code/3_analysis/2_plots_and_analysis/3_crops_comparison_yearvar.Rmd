...
if (months_ts == TRUE){
  ...
  data <- data %>% group_by(x, y, year) %>% summarize(ET = mean(ET, na.rm=TRUE), 
...
data <- merge(data, crops, by = c("x", "y", "year"), all.x=TRUE)
data <- data %>% filter(!is.na(cropnames)) # only keep data that has crop info (get rid of years without dwr data)
...
vario_ET <- variogram(ET~1, data=sample, locations= ~x+y, cutoff= 50) # cutoff 
vario_ag_ET <- variogram(ag_ET~1, data=sample, locations= ~x+y, cutoff= 50) # cutoff 
...
ggplot(ag_ET_df, aes(x = reorder(cropnames, ET))) + 
  geom_linerange(data = ag_ET_df, aes(ymax = min*scalar, ymin = max*scalar), color = "seagreen") + 
  geom_col(data = bardata2, aes(y = ET*scalar, fill = type), alpha = .6) +
  ...
  geom_linerange(data = ET_df, aes(ymax = min*scalar, ymin = max*scalar), color = "navyblue") + 
  geom_linerange(data = ag_ET_df, aes(ymax = min*scalar, ymin = max*scalar), color = "seagreen") + 
  ylab("cm/year") + 
  theme_classic() + 
  labs(fill='') + 
