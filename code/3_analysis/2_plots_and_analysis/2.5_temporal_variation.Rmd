...
ggplot(bardata, aes(x = month)) +
  ...
  scale_color_manual(
    values = c("ag_ET" = "green4", "ET_pred" = "darkgoldenrod", "ET" = "black"),
...
```{r}
# plot the dataset with the different facets by county with CalSIMETAW.  
...
CS <- CS %>% 
  ...
  summarize(Pcp = mean(Pcp))

avg_yearly_pcp <- mean(CS$Pcp)*12
write(paste("Average yearly precipitation according to CalSIMETAW (mm/year):", avg_yearly_pcp),file=report,append=TRUE)
...
ggplot(bardata, aes(x = month)) +
  ...
  scale_x_continuous(breaks = c(1:12)) +
  scale_color_manual(
    values = c("ag_ET" = "green4", "ET_pred" = "darkgoldenrod", "ET" = "black", "Pcp" = "grey"),
    breaks = c("ag_ET", "ET_pred", "ET", "Pcp"),
    labels = c("Agricultural ET", "Natural ET", "Total ET", "Precipitation"),
    name = ""
  ) +
  scale_fill_manual(
    values = c("ag_ET" = "green4", "ET_pred" = "darkgoldenrod", "ET" = "black", "Pcp" = "white"),
    breaks = c("ag_ET", "ET_pred", "ET", "Pcp"),
    labels = c("Agricultural ET", "Natural ET", "Total ET", "Precipitation"),
    name = ""
  ) +
...
ggsave(here(experiment_path, "Figure_1.5_counties_pcp.pdf"), 
       width = 17.8,
       height = 11.1,
       units = "cm")
```
...
```{r}
# break down by sacramento and san joaquin valleys

sac_counties <- c("Tehama", "Glenn", "Butte", "Colusa", "Sutter", "Yuba", "Yolo", "Solano", "Sacramento")
sj_counties <- c("San Joaquin", "Stanislaus", "Merced", "Madera", "Fresno", "Kings", "Tulare", "Kern")
data$valley <- ifelse(data$NAME %in% sac_counties, "Sacramento Valley", "San Joaquin Valley")

valleys <- unique(data$valley)
bardata <- rbindlist(lapply(valleys, make_df, "valley"))
...
ggplot(bardata, aes(x = month)) +
  ...
  facet_wrap(vars(valley))

ggsave(here(experiment_path, "Figure_1.5_valleys.pdf"), 
       width = 17.8,
       height = 11.1,
       units = "cm")
```
