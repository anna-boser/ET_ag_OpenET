...
# simetaw-rain (monthly) vs ag ET (yearly)
```{r, eval = FALSE}
ggplot(scatter_data) + 
  geom_abline(intercept=0,slope=1, color="red") + 
  geom_point(aes(x = ag_ET, y = ag_ET_need)) + 
  geom_smooth(aes(x = ag_ET, y = ag_ET_need), method="lm") + 
  theme_classic() + 
  xlab("Empirical Agricultural ET (cm/year)") + 
  ylab("CalSIMETAW Agricultural ET (cm/year)") +
  coord_fixed() + 
  xlim(c(min(scatter_data$ag_ET_need, scatter_data$ag_ET), max(scatter_data$ag_ET_need, scatter_data$ag_ET))) +
  ylim(c(min(scatter_data$ag_ET_need, scatter_data$ag_ET), max(scatter_data$ag_ET_need, scatter_data$ag_ET)))

lm(ag_ET_need~ag_ET, scatter_data) %>% summary() %>% print()

bias <- mean(scatter_data$ag_ET_need - scatter_data$ag_ET)
se <- sd(scatter_data$ag_ET_need - scatter_data$ag_ET)/sqrt(nrow(scatter_data))
print_CI("simetaw-rain (monthly) vs ag ET (yearly)", bias, se)
print_CI("bias as percent of theoretical", (bias/mean(scatter_data$ag_ET_need))*100, (se/mean(scatter_data$ag_ET_need))*100)
```

# simetaw-ET_pred (monthly) vs ag ET (yearly)
```{r, eval = FALSE}
ggplot(scatter_data) + 
  geom_abline(intercept=0,slope=1, color="red") + 
  geom_point(aes(x = ag_ET, y = ETc_ET_pred)) + 
  geom_smooth(aes(x = ag_ET, y = ETc_ET_pred), method="lm") + 
  theme_classic() + 
  xlab("Empirical Agricultural ET (cm/year)") + 
  ylab("CalSIMETAW Agricultural ET (cm/year)") +   
  coord_fixed() + 
  xlim(c(min(scatter_data$ETc_ET_pred, scatter_data$ag_ET), max(scatter_data$ETc_ET_pred, scatter_data$ag_ET))) +
  ylim(c(min(scatter_data$ETc_ET_pred, scatter_data$ag_ET), max(scatter_data$ETc_ET_pred, scatter_data$ag_ET)))

lm(ETc_ET_pred~ag_ET, scatter_data) %>% summary()

bias <- mean(scatter_data$ETc_ET_pred - scatter_data$ag_ET)
se <- sd(scatter_data$ETc_ET_pred - scatter_data$ag_ET)/sqrt(nrow(scatter_data))
print_CI("simetaw-ET_pred (monthly) vs ag ET (yearly)", bias, se)
print_CI("bias as percent of theoretical", (bias/mean(scatter_data$ETc_ET_pred))*100, (se/mean(scatter_data$ETc_ET_pred))*100)
```
...
ggplot(scatter_data) + 
  geom_abline(intercept=0,slope=1, color="red") + 
  geom_point(aes(x = ET_ET_pred, y = ETc_ET_pred)) + 
  geom_smooth(aes(x = ET_ET_pred, y = ETc_ET_pred), method="lm") + 
  theme_classic() + 
  xlab("Empirical Agricultural ET (cm/year)") + 
  ylab("CalSIMETAW Agricultural ET (cm/year)") + 
  coord_fixed() + 
  xlim(c(min(scatter_data$ETc_ET_pred, scatter_data$ET_ET_pred), max(scatter_data$ETc_ET_pred, scatter_data$ET_ET_pred))) +
  ylim(c(min(scatter_data$ETc_ET_pred, scatter_data$ET_ET_pred), max(scatter_data$ETc_ET_pred, scatter_data$ET_ET_pred)))

lm(ETc_ET_pred~ET_ET_pred, scatter_data) %>% summary()
paste("correlation:", cor(scatter_data$ETc_ET_pred,scatter_data$ET_ET_pred))
...
print_CI("simetaw-ET_pred (monthly) vs ag ET (monthly)", bias, se)
print_CI("bias as percent of theoretical", (bias/mean(scatter_data$ETc_ET_pred))*100, (se/mean(scatter_data$ETc_ET_pred))*100)
```
...
ggplot(scatter_data) + 
  geom_abline(intercept=0,slope=1, color="red") + 
  geom_point(aes(x = ET_ET_pred, y = ETc_ET_pred, color = cropnames)) + 
...
ggplot(scatter_data) + 
  geom_abline(intercept=0,slope=1, color="red") + 
  geom_point(aes(x = ET_ET_pred, y = ag_ET_need)) + 
  geom_smooth(aes(x = ET_ET_pred, y = ag_ET_need), method="lm") + 
  theme_classic() + 
  ylab("CalSIMETAW Agricultural ET (cm/year)") + 
  xlab("Empirical Agricultural ET (cm/year)") + 
  coord_fixed() + 
  xlim(c(min(scatter_data$ag_ET_need, scatter_data$ET_ET_pred), max(scatter_data$ag_ET_need, scatter_data$ET_ET_pred))) +
  ylim(c(min(scatter_data$ag_ET_need, scatter_data$ET_ET_pred), max(scatter_data$ag_ET_need, scatter_data$ET_ET_pred)))

lm(ag_ET_need~ET_ET_pred, scatter_data) %>% summary()
paste("correlation:", cor(scatter_data$ag_ET_need,scatter_data$ET_ET_pred))
...
print_CI("simetaw-rain (monthly) vs ag ET (monthly)", bias, se)
print_CI("bias as percent of theoretical", (bias/mean(scatter_data$ag_ET_need))*100, (se/mean(scatter_data$ag_ET_need))*100)
```
...
ggplot(scatter_data) + 
  geom_abline(intercept=0,slope=1, color="red") + 
  geom_point(aes(x = ET_ET_pred, y = ag_ET_need, color = cropnames)) + 
  scale_color_manual(values=cols) + 
  geom_smooth(aes(x = ET_ET_pred, y = ag_ET_need), method="lm") + 
  theme_classic() + 
  ylab("CalSIMETAW Agricultural ET (cm/year)") + 
...
# ET_aw (monthly) vs ag ET (monthly). Do not use this because the readme does not specify if they assume irrigation efficiency is 0: https://data.ca.gov/dataset/cal-simetaw-unit-values/resource/ca06a0cf-d2a4-433d-99db-672b147afd5d

```{r, eval = FALSE}
ggplot(scatter_data) + 
  geom_abline(intercept=0,slope=1, color="red") + 
  geom_point(aes(x = ET_ET_pred, y = AW)) + 
  geom_smooth(aes(x = ET_ET_pred, y = AW), method="lm") + 
  theme_classic() + 
  ylab("CalSIMETAW Agricultural ET (cm/year)") + 
  xlab("Empirical Agricultural ET (cm/year)") + 
  coord_fixed() + 
  xlim(c(min(scatter_data$AW, scatter_data$ET_ET_pred), max(scatter_data$AW, scatter_data$ET_ET_pred))) +
  ylim(c(min(scatter_data$AW, scatter_data$ET_ET_pred), max(scatter_data$AW, scatter_data$ET_ET_pred)))

lm(AW~ET_ET_pred, scatter_data) %>% summary()
paste("correlation:", cor(scatter_data$AW,scatter_data$ET_ET_pred))

bias <- mean(scatter_data$AW - scatter_data$ET_ET_pred)
se <- sd(scatter_data$AW - scatter_data$ET_ET_pred)/sqrt(nrow(scatter_data))
print_CI("ET_aw (monthly) vs ag ET (monthly)", bias, se)
print_CI("bias as percent of theoretical", (bias/mean(scatter_data$AW))*100, (se/mean(scatter_data$AW))*100)
```
...
ggplot(scatter_data) + 
  geom_abline(intercept=0,slope=1, color="red") + 
  geom_point(aes(x = ET_ET_pred, y = AW, color = cropnames)) + 
  scale_color_manual(values=cols) + 
  geom_smooth(aes(x = ET_ET_pred, y = AW), method="lm") + 
  theme_classic() + 
  ylab("CalSIMETAW Agricultural ET (cm/year)") + 
...
ggplot(scatter_data) + 
  geom_abline(intercept=0,slope=1, color="red") + 
  geom_point(aes(x = ET_pred, y = Pcp)) + 
  geom_smooth(aes(x = ET_pred, y = Pcp), method="lm") + 
  theme_classic() + 
  xlab("Simulated naturally-occurring ET (cm/year)") + 
  ylab("CalSIMETAW Precipitation (cm/year)") + 
  coord_fixed() + 
  xlim(c(min(scatter_data$ET_pred, scatter_data$Pcp), max(scatter_data$ET_pred, scatter_data$Pcp))) +
  ylim(c(min(scatter_data$ET_pred, scatter_data$Pcp), max(scatter_data$ET_pred, scatter_data$Pcp)))

lm(Pcp~ET_pred, scatter_data) %>% summary()

bias <- mean(scatter_data$Pcp - scatter_data$ET_pred)
se <- sd(scatter_data$Pcp - scatter_data$ET_pred)/sqrt(nrow(scatter_data))
print_CI("rain vs naturally-occurring ET bais", bias, se)
```
