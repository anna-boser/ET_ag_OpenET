...
data_year <- data %>% group_by(x, y, year) %>% summarize(ET = mean(ET, na.rm=TRUE)*scalar, 
...
ggplot(data_year) +
  ...
  xlab("Predicted natural ET (cm/year)") + 
  ylab("Observed fallow ET (cm/year)") +
    xlim(c(0,85*scalar)) + 
    ylim(c(0,85*scalar))
...
new.labs <- c("Observed ET", "Simulated ET")
...
loc_df %>% filter(type %in% c("Bias")) %>%
  ...
  scale_color_distiller(name="ET (cm/year)", palette="RdBu", limits = c(-55, 55)) + 
  theme_void()
```
...
ET_pred_df$type = "Simulated fallow ET"
...
ggplot(ts_bias, aes(x = month)) + 
  geom_line(aes(y = ET)) + 
  geom_ribbon(aes(ymax = min, ymin = max), alpha = .2) +
  xlab("Month") + 
  ylab("ET (mm/month)")
```
